...
public class DynamicHandler implements HttpHandler {
    ...
    private Res doGet(HttpExchange httpExchange) {
    ...
    private Res doPost(HttpExchange exchange) {
        // リクエストメッセージからボディ部を取り出し、数値データに変換する
        try(Scanner scanner = new Scanner(exchange.getRequestBody(), "UTF-8")) {
            String scanned = scanner.nextLine();        // 要求ボディの取り出し
            String[] split = scanned.split("=");  // 変数名と値(yenとxxx)に分離
            int yen = Integer.parseInt(split[1]);       // 値(yen)を数値に変換
            float usd = yen * .0095f;                   // データ処理：円をドルに変換する
            // 処理結果のリソース作成
            String body = String.format("<p>%d 円は %.2f ドル</p>", yen, usd);
            return new Res(200, body);
        } catch(RuntimeException e) {
            e.printStackTrace();
            return new Res(500, "<p>500 サーバー内部でエラーが生じました</p>");
        }
    }
}
